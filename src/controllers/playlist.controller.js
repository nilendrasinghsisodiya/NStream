...
const addVideoToPlaylist = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, updatedPlaylist, 'Videos added successfully to the playlist'));
});
...
const removeVideoFromPlaylist = asyncHandler(async (req, res) => {
  ...
  if (!req?.user?._id) {
    throw new ApiError(401, 'you need to be logged in to remove a video from a playlist');
  }
  ...
  if (!isOwner) {
    throw new ApiError(403, 'you need to be owner to remove a video from a playlist');
  }
  const updatedPlaylist = await Playlist.findByIdAndUpdate(
    playlistId,
    {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, updatePlaylist, 'video removed from playlist sucessfully'));
});
