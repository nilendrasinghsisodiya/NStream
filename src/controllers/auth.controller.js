...
const registerUser = asyncHandler(async (req, res) => {
  ...
  console.log('user');
  const userExist = await User.findById(user._id).select('-password -refreshToken -watchHistory');
  ...
  if (email) {
    ...
    if (userExist) {
      ...
      if (validPassword) {
        ...
        const result = { ...loggedInUser._doc };
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  try {
    ...
    throw new ApiError(401, error.message || 'something went wrong when generating tokens');
  }
});
